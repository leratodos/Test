...
function appAge() {
  ...
  let data = setInterval(() => {
    ...
    if (empty_field != "" && full_month != "" && year != "") {
      let selectS = "" + empty_field + "." + (full_month <10 ? `0${full_month}` : full_month) + "." + year + "";
...
function YsNo() {
  scrollDown();
  $(".chat-content-list").append(
    '<div class="chat-content-buttons-gender"><div class="chat-content-buttons-gender-block"><span class="chooseGenderM yeas" id="yeas">Да</span></div><div class="chat-content-buttons-gender-block"><span class="chooseGenderW no" id="no">Нет</span></div></div>'
  );
...
function Photofoot() {
  $(".chat-content-list").append(
    '<div class="chat-content-item manager img_block"><img class="symptomsImg" style="max-width: 625px;" src="images/symptomsFull.png" id="foot1"><img class="symptoms-mobile" src="images/symptoms.png"></div>'
  );
  $('.img_block').css('margin-bottom', '10px');
...
function validateNumber (element) {
  const phone_number = element.val();
  const item = element.parents('.item');
  var phone = new RegExp(/^\+?3?8?(0[5-9][0-9]\d{7})$/)
  $('.validation_message').remove()
  if (phone.test(phone_number)) {
    return true 
  } else {
    item.append('<div class="validation_message">Пожалуйста, введите валидный номер телефона</div>');
  }
}
...
function showForm() {
  ...
  inputPhone.on('input', function(){
    validateNumber(inputPhone)
  });

  $(document).on('click', '.submit-roulette', function(e) {
    e.preventDefault();
...
function getInfo () {
  const userInfo = Array.from($(".user").children("p").map(function(){
    return $(this).text();
  }))
  return userInfo
};
